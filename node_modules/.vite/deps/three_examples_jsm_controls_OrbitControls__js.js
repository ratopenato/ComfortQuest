import { EventDispatcher, MOUSE, MathUtils, Plane, Quaternion, Ray, Spherical, TOUCH, Vector2, Vector3 } from "./three.js";
//#region ../../../Downloads/threejs-office-interactive-updated/node_modules/three/examples/jsm/controls/OrbitControls.js
...
var OrbitControls = class extends EventDispatcher {
	constructor(object, domElement) {
		...
		this.update = function() {
			...
			return function update(deltaTime = null) {
				...
				if (scope.zoomToCursor && performCursorZoom) {
					...
					if (newRadius !== null) if (this.screenSpacePanning) scope.target.set(0, 0, -1).transformDirection(scope.object.matrix).multiplyScalar(newRadius).add(scope.object.position);
					else {
		...
		function getZoomScale(delta) {
			...
			return Math.pow(.95, scope.zoomSpeed * normalizedDelta);
		}
		...
		function handleTouchMoveDolly(event) {
			...
			dollyStart.copy(dollyEnd);
			updateZoomParameters((event.pageX + position.x) * .5, (event.pageY + position.y) * .5);
		}
		...
		function interceptControlDown(event) {
			if (event.key === "Control") {
				controlActive = true;
				scope.domElement.getRootNode().addEventListener("keyup", interceptControlUp, {
					passive: true,
					capture: true
				});
			}
		...
		function interceptControlUp(event) {
			if (event.key === "Control") {
				controlActive = false;
				scope.domElement.getRootNode().removeEventListener("keyup", interceptControlUp, {
					passive: true,
					capture: true
				});
			}
		...
		function getSecondPointerPosition(event) {
			return pointerPositions[event.pointerId === pointers[0] ? pointers[1] : pointers[0]];
		}
		scope.domElement.addEventListener("contextmenu", onContextMenu);
		scope.domElement.addEventListener("pointerdown", onPointerDown);
		scope.domElement.addEventListener("pointercancel", onPointerUp);
		scope.domElement.addEventListener("wheel", onMouseWheel, { passive: false });
		scope.domElement.getRootNode().addEventListener("keydown", interceptControlDown, {
			passive: true,
			capture: true
		});
		this.update();
